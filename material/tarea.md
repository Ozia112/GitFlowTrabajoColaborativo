...
3. **Decidir cuál encaja mejor con su proyecto y justificarlo** con los factores vistos en clase: tamaño del equipo y del proyecto, tiempo, burocracia, trazabilidad, costo de un error, entre otros. Vale elegir lo mismo que se hizo en clase, algo distinto o una mezcla, siempre que esté justificado.
...
- La estrategia elegida y **por qué** (y por qué no las otras).
- Sus políticas concretas:
  - cómo se nombran las ramas;
  - cómo se escriben los commits;
  - qué necesita un PR para integrarse (revisores, robot, descripción);
  - cómo se integra (merge, squash o rebase);
  - qué ramas se protegen y con qué reglas;
  - qué revisaría un robot si tuvieran uno;
  - cómo resuelven conflictos y cómo deshacen errores.
- **Qué reglas decidieron no tener** y por qué. Es igual de importante que las que sí tienen.
...
| Actividad | Tiempo |
|---|---|
| Acordar la idea y el contexto del proyecto | 30 min |
| Investigación | 90 min |
| Escribir el documento a través de issues y PRs | 60 min |
| Diapositivas y grabación | 45 min |
| Ajustar la configuración del repo y revisar la entrega | 15 min |
